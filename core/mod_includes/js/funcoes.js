function shareFacebook(url) {
  ...
  window.open(
    ...
    "toolbar=no, location=no, directories=no, status=no, " +
      "menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width=" +
      w +
      ", height=" +
      h +
      ", top=" +
...
function moeda(obj, e) {
  ...
  if ((tecla != 46 && tecla < 48) || tecla > 57) return false;
}
...
function mascaraData(inputData, e) {
  ...
  if (data.length == 10) {
    ...
    if (tecla >= 48 && tecla < 58) {
      ...
      // Backspace, Delete e setas direcionais(para mover o cursor, apenas para FF)
      return true;
    else return false;
  }
}
...
function SomenteNumeroCEL(inputData, e) {
  ...
  if (campo.length == 15) {
    ...
    var tecla = window.event ? event.keyCode : e.which;
    if ((tecla > 47 && tecla < 58) || tecla == 32) return true;
    else {
      if (tecla != 8) return false;
      else return true;
    }
...
function aumentarZoom() {
  if (
    window.parent.document.body.style.zoom != undefined &&
